...
public class RoomAvailabilityControllerTest {
    ...
    void shouldReturnAvailableRoomsSuccessfully() throws Exception {
        ...
        RoomAvailabilityResponseDto response = new RoomAvailabilityResponseDto(1L, hotelId, 2L, startDate,
                availableQuantity, BigDecimal.valueOf(100.00));

        when(service.getAvailableRooms(hotelId, roomTypeId, startDate, endDate, availableQuantity))
                .thenReturn(List.of(response));

        mockMvc.perform(get("/api/room-availability?hotelId=1&roomTypeId=2&startDate=2026-05-01&endDate=2026-05-02&availableQuantity=2")
    ...
    @Test
    void shouldReturn400_WhenDateRangeIsInvalid() throws Exception {
        mockMvc.perform(get("/api/room-availability?hotelId=1&startDate=2026-05-05&endDate=2026-05-01&quantity=1"))
